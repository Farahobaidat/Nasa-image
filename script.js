...
Search.addEventListener("click",(event)=>{
    ...
    <h4>${image.title} <br>${image.date}</h4>
        ...
        showImages.insertAdjacentHTML("beforeend",`<div class="image">
        <h2>${image.media_type}</h2><iframe src="${image.thumbnail_url}" class="images" poster="
        https://i.pinimg.com/originals/f9/9b/2e/f99b2e7cace21a55b6eafb37c569bb01.jpg‏" controls></iframe>
        <h4>${image.title} <br>${image.date}</h4>
        </div>`) 
    }
    }})
...
showImages.onclick=function(event){
    details.innerHTML="";
       let click=data.filter((deta)=>{if(event.target.parentNode.children[1].src==deta.url){return deta}})
      ...
      <iframe src="${click[0].thumbnail_url}"></iframe><h6>${copyrights}</h6> </div>`)}
